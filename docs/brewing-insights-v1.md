...
- Weekday bars use a 200px plot on desktop/tablet and 160px on phones. The number above each pair is the selected period's total for that weekday (all Mondays in a 28-day view, for example), not an average or a sum of both periods; align it over the current-period bar. Light charts share a cooler green current fill, neutral grey comparison fill and light neutral clock face. Keep UI text/accent colors separate from the chart palette.
