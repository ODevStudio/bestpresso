...
import { timeWindows, timeWindowCounts, type HistoryRecord } from './historyData'
...
export function TimeCoxcomb({ current, previous, onOpen, comparison }: TimeCoxcombProps) {
  const counts = [timeWindowCounts(current), timeWindowCounts(previous)]
  const maximum = Math.max(1, ...counts.flat())
  const [period, setPeriod] = useState(0)
  const [selected, setSelected] = useState<number | null>(null)
  const periodNames = comparison ? ['This period', 'Previous'] : ['This period']
