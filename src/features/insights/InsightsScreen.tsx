...
import { coversWindow, dateLabel, inWindow, matches, reportingWindow, shiftDate, summarize, timeLabel, timeWindows, weekdays, type HistoryRecord, type InsightFilter } from './historyData'
import type { InsightsRoute } from './insightsRoute'
...
export function InsightsScreen({ data, route, navigate, onClose }: { data: ShotInsights; route: InsightsRoute; navigate: (next: InsightsRoute, replace?: boolean) => void; onClose: () => void }) {
  ...
  const { days, section, beverage, filter, search, previous, shotId } = route
  ...
  const patch = (values: Partial<InsightsRoute>, replace = false) => navigate({ ...route, ...values }, replace)
  ...
  const displayPeriod = days === 0 ? `Latest ${records.length} saved records` : `${dateLabel((previous && section === 'history' ? priorWindow : window).start)} – ${dateLabel(shiftDate((previous && section === 'history' ? priorWindow : window).end, -1))}`
  const filterText = !filter ? 'All brews' : filter.kind === 'weekday' ? weekdays[Number(filter.value)] : filter.kind === 'hours' ? timeWindows[Number(filter.value)].label : records.find(r => r.profileKey === filter.value)?.profile ?? 'Selected profile'
  const syncText = cache ? `${data.status === 'offline' ? 'Offline · ' : ''}Last synced ${new Date(cache.syncedAt).toLocaleString()} · ${records.length} of ${cache.total} records saved` : data.status === 'loading' ? 'Loading saved history from Decaid…' : 'Decaid history is unavailable. Connect to load your first 100 records.'
  ...
  return <div className="ins-theme ins-app"><div className="ins-shell"><aside className="ins-rail"><SidebarBrand onClose={onClose} closeLabel="Close insights"/><small className="ins-eyebrow">YOUR BREWING</small><nav aria-label="Insights navigation"><SidebarNavItem active={section === 'overview'} onClick={() => { savedScroll.current = 0; patch({ section: 'overview', days: days || 7, filter: null, previous: false, beverage: beverage === 'all' ? 'espresso' : beverage }) }}><span aria-hidden="true">◫</span> Overview</SidebarNavItem><SidebarNavItem active={section === 'history'} onClick={() => openHistory()}><span aria-hidden="true">◷</span> History</SidebarNavItem></nav><div className="ins-rail-foot">Saved in Decaid<small>Latest 100 records<br/>Cached on this device</small></div></aside>
    <div className="ins-content" ref={scroll}><header className="ins-heading"><div><h1>{section === 'overview' ? 'Your brewing' : 'Brew history'}</h1><p>{displayPeriod} · {drinkNames[beverage]}</p></div><div className="ins-heading-controls"><label className="ins-period"><span className="ins-sr">Drink type</span><select value={beverage} onChange={e => patch({ beverage: e.target.value as InsightsRoute['beverage'], filter: null })}><option value="espresso">Espresso</option><option value="pourover">Pour-over</option><option value="other">Other / unknown</option>{section === 'history' && <option value="all">All activities</option>}</select></label><label className="ins-period"><span className="ins-sr">Period</span><select value={days} onChange={e => patch({ days: Number(e.target.value) as InsightsRoute['days'], filter: null, previous: false })}><option value="7">Last 7 days</option><option value="28">Last 28 days</option>{section === 'history' && <option value="0">All cached history</option>}</select></label></div></header>
      <div className="ins-sync"><div><p role="status">{syncText}</p>{data.storageWarning && <p>Offline storage is unavailable. This session’s data may not survive closing the app.</p>}{days !== 0 && !displayedComplete && <p>Limited date coverage — showing cached records, not a complete period.</p>}{section === 'overview' && !comparison && <p>Not enough history for comparison.</p>}</div><button className="ins-link" disabled={data.refreshing} onClick={() => void repository.refresh()}>{data.refreshing ? 'Refreshing…' : 'Refresh'}</button></div>
      {section === 'overview' ? <>
        <div className="ins-top-grid"><section className="ins-chart-section"><header><h2>By day</h2><div className="ins-legend"><span><i/>This period</span>{comparison && <span><i/>Previous</span>}</div></header><div className="ins-week" aria-label="Brews by weekday">{weekdays.map((name, i) => { const max = Math.max(1, ...summary.weekdays, ...(comparison ? before.weekdays : [])); return <button key={name} onClick={() => openHistory({ kind: 'weekday', value: String(i) })} aria-label={`${name}: ${summary.weekdays[i]} cached brews. View shots`}><span className="ins-week-value"><span>{cache ? summary.weekdays[i] : '—'}</span></span><span className="ins-bar-pair">{comparison ? <i data-empty={before.weekdays[i] === 0} style={{ height: `${before.weekdays[i] / max * 100}%` }}/> : <i style={{ visibility: 'hidden' }}/>}<b data-empty={summary.weekdays[i] === 0} style={{ height: `${summary.weekdays[i] / max * 100}%` }}/></span><small>{name}</small></button> })}</div></section><TimeCoxcomb key={`${days}-${beverage}-${comparison}`} current={current} previous={comparison ? prior : []} comparison={comparison} onOpen={(i, earlier) => openHistory({ kind: 'hours', value: String(i) }, earlier)}/></div>
      ...
      </> : <><div className="ins-history-toolbar"><div><span className="ins-filter">{filterText}{filter && <button aria-label="Clear insight filter" onClick={() => patch({ filter: null })}>×</button>}</span>{days !== 0 && comparison && <div className="ins-evidence-switch"><button aria-pressed={!previous} onClick={() => patch({ previous: false })}>This period</button><button aria-pressed={previous} onClick={() => patch({ previous: true })}>Previous period</button></div>}</div><label><span className="ins-sr">Search profiles</span><input placeholder="Search profiles" value={search} onChange={e => patch({ search: e.target.value }, true)}/></label></div><section className="ins-panel ins-history-list"><header><div><h2>{history.length} recorded brews</h2><p>{summarize(history).yieldCoverage} with yield readings</p></div><small>Tap a brew to analyse ↗</small></header>{rows(history)}</section></>}
