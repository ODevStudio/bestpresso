  ...
  const prepare = <div className="settings-grid">
    <SettingsCard eyebrow="Drink settings" title="Stopping calibration" description="Calibrate espresso and hot water separately. Higher values stop earlier.">
      <SectionList>
        ...
        <NumberSetting label="Volume" hint="Machine-flow projection" value={numberValue(settings.draft.rea.volumeFlowMultiplier)} unit="s" min={0} step={0.05} digits={2} disabled={reaUnavailable} onChange={(volumeFlowMultiplier) => settings.patchRea({ volumeFlowMultiplier })} />
      </SectionList>
      <button className="settings-reset" type="button" disabled={reaUnavailable || settings.draft.rea.hotWaterFlowMultiplier === 0.3} onClick={() => settings.patchRea({ hotWaterFlowMultiplier: 0.3 })}>Reset hot-water calibration to 0.3 s</button>
    </SettingsCard>
