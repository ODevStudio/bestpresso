...
import { BENGLE_WATER_WARNING_MM, isBengle } from '../machine/waterTelemetry'
...
export function SettingsScreen({ model, connection, machineConnection, scale, onClose }: SettingsScreenProps) {
  const bengle = isBengle(useMachineSession().info?.model)
  ...
  const overview = <div className="settings-overview">
    ...
    <SettingsCard eyebrow={t('settings.group.bestpresso')} title={t('settings.overview.feedback.title')} description={t('settings.overview.feedback.description')}><div className="settings-list"><div className="settings-row"><span>{t('settings.overview.feedback.completionSound')}</span><strong>{preferences.completionSoundEnabled ? t('settings.state.on') : t('settings.state.off')}</strong></div><div className="settings-row"><span>{t('settings.overview.feedback.chartLines')}</span><strong>{chartLineWeightLabel(preferences.chartLineWeight)}</strong></div><div className="settings-row"><span>{t('settings.overview.feedback.waterWarning')}</span><strong>{bengle ? `${BENGLE_WATER_WARNING_MM} mm` : `${preferences.waterWarningLevelMl} ml`}</strong></div></div><button className="settings-card-link" type="button" onClick={() => setActiveSection('experience')}>{t('settings.action.personalize')} <span>→</span></button></SettingsCard>
  ...
  const alerts = <div className="settings-grid settings-grid--alerts">
    <SettingsCard eyebrow={t('settings.group.bestpresso')} title={t('settings.alerts.reservoir.title')} description={t('settings.alerts.reservoir.description')} action={bengle ? undefined : <button className="settings-reset" type="button" onClick={() => updatePreferences({ waterWarningLevelMl: DEFAULT_BESTPRESSO_PREFERENCES.waterWarningLevelMl })}>{t('settings.action.reset')}</button>}><SectionList>{bengle ? <div className="settings-control-row"><strong>{t('settings.alerts.warnMe')}</strong><span>{BENGLE_WATER_WARNING_MM} mm</span></div> : <NumberSetting label={t('settings.alerts.warnMe')} hint={t('settings.alerts.warnMeHint')} value={preferences.waterWarningLevelMl} unit="ml" min={0} max={2000} step={10} onChange={updateWarning} />}</SectionList><p className="settings-helper">{t('settings.alerts.priorityHelper')}</p></SettingsCard>
