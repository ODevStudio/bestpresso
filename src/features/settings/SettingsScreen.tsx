...
const sections: { id: SettingsSection; label: string; group: string; keywords: string }[] = [
  ...
  { id: 'experience', label: 'App experience', group: 'Bestpresso', keywords: 'sound chart thickness theme appearance animations motion performance' },
...
export function SettingsScreen({ model, connection, machineConnection, scale, onClose }: SettingsScreenProps) {
  ...
  const prepare = <div className="settings-grid">
    <SettingsCard eyebrow="Drink settings" title="Stopping calibration" description="Calibrate espresso and hot water separately. Higher values stop earlier.">
      <SectionList>
        <NumberSetting label="Espresso yield" value={numberValue(settings.draft.rea.weightFlowMultiplier)} min={0} step={0.1} digits={1} disabled={reaUnavailable} onChange={(weightFlowMultiplier) => settings.patchRea({ weightFlowMultiplier })} />
        <NumberSetting label="Hot-water yield" hint="Decaid default: 0.3 s. Zero uses that default, not no prediction." value={numberValue(settings.draft.rea.hotWaterFlowMultiplier)} unit="s" min={0} step={0.05} digits={2} disabled={reaUnavailable} onChange={(hotWaterFlowMultiplier) => settings.patchRea({ hotWaterFlowMultiplier })} />
        <NumberSetting label="Volume" hint="Shot stopping without a scale" value={numberValue(settings.draft.rea.volumeFlowMultiplier)} unit="s" min={0} step={0.05} digits={2} disabled={reaUnavailable} onChange={(volumeFlowMultiplier) => settings.patchRea({ volumeFlowMultiplier })} />
      </SectionList>
    </SettingsCard>
