...
for (const type of ['pressure', 'flow'] as const) for (const condition of ['over', 'under'] as const) {
  test(`${type} ${condition}: recognizes a near, trending crossing on the first new-frame sample`, () => {
    ...
    assert.equal(result(samples(values, type), { exit: { type, condition, value: 4 } }).label, `${type === 'pressure' ? 'Pressure' : 'Flow'} threshold reached`)
...
test('compares duration and sensor exits with the encoded firmware values', () => {
  ...
  assert.equal(result(samples([3,3.9,4.0625,4.1]), {exit:{type:'pressure',condition:'over',value:4.08}}).label, 'Pressure threshold reached')
})
test('natural final-frame completion uses profile exits without overriding explicit stops', () => {
  ...
  assert.equal(result(p,step,{stopReason:'machineEnded'}).label,'Time limit reached or Pressure threshold reached')
...
test('numeric strings, recorded priority, and OR remain supported', () => {
  ...
  assert.equal(result(p,step).label,'Time limit reached or Pressure threshold reached')
