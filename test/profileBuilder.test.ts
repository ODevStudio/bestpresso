...
test('cancel protects a changed profile draft while leaving an untouched draft immediately closable', () => {
  ...
  assert.match(screen, /t\('builder\.discard\.title'\)/)
  assert.equal(en['builder.discard.title'], 'Discard profile changes?')
...
test('every numeric stage setting opens the shared fullscreen ruler', () => {
  ...
  assert.match(screen, /t\('builder\.stepper\.openFullscreenLabel', \{ label \}\)/)
  assert.equal(en['builder.stepper.openFullscreenLabel'], 'Open {label} fullscreen adjustment')
...
test('editor follows the designed high-level hierarchy without prototype-only fields', () => {
  assert.match(screen, /pb-stage__conditions-rule"><img src=\{skipNext\} alt="" \/><span>\{t\('builder\.stage\.conditionsHint'\)\}<\/span><\/p>/)
  assert.equal(en['builder.stage.conditionsHint'], 'The next stage starts as soon as any condition on the left is met.')
  assert.match(screen, /label: t\('builder\.profile\.endShotYield'\)/)
  assert.match(screen, /<span>\{t\('builder\.profile\.endShotYield'\)\} <img/)
  assert.equal(en['builder.profile.endShotYield'], 'End shot yield')
  assert.match(screen, /<small>\{t\('builder\.stage\.moveOnVolumeLabel'\)\}<\/small>/)
  assert.match(screen, /<small>\{t\('builder\.stage\.moveOnYieldLabel'\)\}<\/small>/)
  assert.equal(en['builder.stage.moveOnVolumeLabel'], 'Move on volume')
  assert.equal(en['builder.stage.moveOnYieldLabel'], 'Move on yield')
  assert.match(screen, /label=\{t\('builder\.stage\.moveOnVolumeLabel'\)\}/)
  assert.match(screen, /label=\{t\('builder\.stage\.moveOnYieldLabel'\)\}/)
  assert.match(screen, /const label = type === 'flow' \? t\('builder\.exit\.moveOnFlow'\) : t\('builder\.exit\.moveOnPressure'\)/)
  assert.equal(en['builder.exit.moveOnFlow'], 'Move on flow')
  assert.equal(en['builder.exit.moveOnPressure'], 'Move on pressure')
  ...
  assert.match(screen, /t\('builder\.stage\.measureFromLabel'\)/)
  assert.equal(en['builder.stage.measureFromLabel'], 'Measure from')
  assert.match(screen, /t\('builder\.profile\.uncategorized'\)/)
  assert.equal(en['builder.profile.uncategorized'], 'Uncategorized')
  assert.doesNotMatch(screen, /Prototype only/)
  assert.doesNotMatch(screen, />Limiter range</)
  assert.match(screen, /aria-label=\{t\('builder\.stage\.addStage'\)\}/)
  assert.equal(en['builder.stage.addStage'], 'Add stage')
  assert.match(screen, /nextBuilderStage/)
})

test('editor panels use a true close icon rather than the add-stage plus', () => {
  assert.match(screen, /aria-label=\{t\('builder\.validation\.closeLabel'\)\}><img src=\{builderCardClose\}/)
  assert.equal(en['builder.validation.closeLabel'], 'Close validation')
...
test('the expanded title card exposes profile prerequisites without repeating title-bar settings', () => {
  ...
  assert.match(screen, /t\('builder\.details\.panelLabel'\)/)
  assert.equal(en['builder.details.panelLabel'], 'Profile details and advanced settings')
  const details = screen.slice(screen.indexOf('{profileDetailsOpen && <section'), screen.indexOf('</section>}\n    </header>'))
  assert.match(screen, /<EditableChoice id="category" label=\{t\('builder\.profile\.categoryLabel'\)\}/)
  assert.equal(en['builder.profile.categoryLabel'], 'Category')
  assert.match(details, /label=\{t\('builder\.details\.versionLabel'\)\}/)
  assert.equal(en['builder.details.versionLabel'], 'Profile version')
  assert.match(screen, /knownCategories/)
  assert.match(screen, /knownVersions/)
  assert.match(details, />\{t\('builder\.details\.authorLabel'\)\}</)
  assert.equal(en['builder.details.authorLabel'], 'Author')
  assert.match(details, /t\('builder\.details\.authorHint'\)/)
  assert.equal(en['builder.details.authorHint'], 'Set from the signed-in account when saved.')
  assert.match(details, />\{t\('builder\.details\.descriptionLabel'\)\}</)
  assert.equal(en['builder.details.descriptionLabel'], 'Description')
  assert.match(screen, /<span>\{t\('builder\.profile\.endShotYield'\)\} <img/)
  assert.match(details, /label=\{t\('builder\.details\.endShotVolumeFallbackLabel'\)\}/)
  assert.equal(en['builder.details.endShotVolumeFallbackLabel'], 'End shot volume fallback')
  assert.match(details, /volumeFallbackActive && <label className="pb-profile-details__measure-from"/)
  assert.match(details, />\{t\('builder\.details\.volumeStartLabel'\)\}</)
  assert.equal(en['builder.details.volumeStartLabel'], 'Start measuring volume from')
  assert.match(details, /<option value="" disabled>\{t\('builder\.details\.chooseStage'\)\}<\/option>/)
  assert.equal(en['builder.details.chooseStage'], 'Choose a stage')
  assert.match(details, /\{index \+ 1\}\. \{stage\.name\.trim\(\) \|\| t\('builder\.stage\.numberLabel', \{ n: index \+ 1 \}\)\}/)
  assert.equal(en['builder.stage.numberLabel'], 'Stage {n}')
  assert.match(details, /label=\{t\('builder\.details\.flowToleranceLabel'\)\}/)
  assert.match(details, /label=\{t\('builder\.details\.pressureToleranceLabel'\)\}/)
  ...
  assert.match(screen, /className="pb-profile-details-backdrop" aria-label=\{t\('builder\.profile\.closeMoreSettings'\)\}/)
  assert.equal(en['builder.profile.closeMoreSettings'], 'Close more settings')
...
test('advanced settings opens from a compact title-row disclosure', () => {
  ...
  assert.match(screen, /profileDetailsOpen \? t\('builder\.profile\.lessSettings'\) : t\('builder\.profile\.moreSettings'\)/)
  assert.equal(en['builder.profile.lessSettings'], 'Less settings')
  assert.equal(en['builder.profile.moreSettings'], 'More settings')
...
test('more-settings metrics reuse stage steppers and open the isolated adjustment screen', () => {
  assert.match(screen, /<SettingsMetric label=\{t\('builder\.details\.endShotVolumeLabel'\)\}/)
  assert.equal(en['builder.details.endShotVolumeLabel'], 'End shot volume (without scale)')
  assert.match(screen, /<SettingsMetric label=\{t\('builder\.details\.flowToleranceLabel'\)\}/)
  assert.match(screen, /<SettingsMetric label=\{t\('builder\.details\.pressureToleranceLabel'\)\}/)
  assert.equal(en['builder.details.flowToleranceLabel'], 'Flow limiter range')
  assert.equal(en['builder.details.pressureToleranceLabel'], 'Pressure limiter range')
  assert.equal(en['builder.details.limiterRangeHint'], 'Controls the limiter’s response around its limit.')
  assert.doesNotMatch(screen, /<SettingsMetric label="Tank temperature"/)
  assert.match(screen, /onOpen=\{\(\) => openProfileMetric/)
  assert.match(screen, /<Stepper label=\{t\('builder\.details\.flowToleranceLabel'\)\}/)
  assert.match(screen, /<Stepper label=\{t\('builder\.details\.pressureToleranceLabel'\)\}/)
})
...
test('stage sequence remains explicit while one fluid card is edited in place', () => {
  ...
  assert.match(screen, /\{stage\.pump === 'pressure' \? t\('builder\.stage\.pressureTargetLabel'\) : t\('builder\.stage\.flowTargetLabel'\)\}/)
  assert.equal(en['builder.stage.pressureTargetLabel'], 'Pressure Target')
  assert.equal(en['builder.stage.flowTargetLabel'], 'Flow Target')
  assert.match(screen, /typeof limiterValue === 'number' && limiterValue > 0 && <i>\{t\('builder\.common\.max'\)\} \{formatValue\(limiterValue\)\} \{limiterUnit\}/)
  assert.equal(en['builder.common.max'], 'Max')
  assert.match(screen, /\{stage\.sensor === 'water' \? t\('builder\.stage\.waterTemperatureLabel'\) : t\('builder\.stage\.coffeeTemperatureLabel'\)\}/)
  assert.equal(en['builder.stage.waterTemperatureLabel'], 'Water temperature')
  assert.equal(en['builder.stage.coffeeTemperatureLabel'], 'Coffee temperature')
  assert.match(screen, /pb-stage__target-panel/)
  assert.match(screen, /pb-stage__conditions-panel/)
  assert.match(screen, /t\('builder\.stage\.movesOnSummary'\)/)
  assert.equal(en['builder.stage.movesOnSummary'], 'Moves on when any is reached')
  assert.match(screen, /\.filter\(\(condition\): condition is string => Boolean\(condition\)\)\.join\(' \/ '\)/)
  assert.match(screen, /role="tablist" aria-label=\{t\('builder\.stage\.tabsLabel'\)\}/)
  assert.equal(en['builder.stage.tabsLabel'], 'Stage settings')
  assert.match(screen, /role="tabpanel" aria-label=\{t\('builder\.stage\.targetPanelLabel'\)\}/)
  assert.equal(en['builder.stage.targetPanelLabel'], 'Target controls')
  assert.match(screen, /role="tabpanel" aria-label=\{t\('builder\.stage\.conditionsPanelLabel'\)\}/)
  assert.equal(en['builder.stage.conditionsPanelLabel'], 'Move on conditions')
  assert.match(screen, /aria-label=\{t\('builder\.stage\.dragHandle'\)\}/)
  assert.equal(en['builder.stage.dragHandle'], 'Drag to reorder stage')
  assert.match(screen, /t\('builder\.stage\.duplicateLabel', \{ n: index \+ 1 \}\)/)
  assert.match(screen, /t\('builder\.stage\.deleteLabel', \{ n: index \+ 1 \}\)/)
  assert.equal(en['builder.stage.duplicateLabel'], 'Duplicate stage {n}')
  assert.equal(en['builder.stage.deleteLabel'], 'Delete stage {n}')
