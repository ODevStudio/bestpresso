...
function Chart({ seed = 1, large = false }: { seed?: number; large?: boolean }) {
  ...
  return <svg className={large ? 'pl-chart pl-chart-large' : 'pl-chart'} viewBox="0 0 200 110" preserveAspectRatio="none" aria-label="Illustrative pressure and flow targets" role="img">
    {large && [20,50,80,108].map(y => <line key={y} x1="0" x2="200" y1={y} y2={y} stroke="var(--ins-line)" vectorEffect="non-scaling-stroke"/>)}
  ...
  return <div className="ins-theme pl-app">
    {!detail ? <div className="ins-shell">
      <aside className="ins-rail">
        ...
        <div className="pl-loaded"><span className="pl-eyebrow">ON YOUR MACHINE</span><strong>{profiles.find(p=>p.id===loaded)?.name || 'No profile loaded'}</strong><span><i/>Loaded profile</span></div>
        <span className="ins-eyebrow">LIBRARY</span>
        <nav aria-label="Profile library"><SidebarNavItem active={section==='All profiles'} onClick={()=>navigate('All profiles')}><Icon name="grid"/>All profiles<small>{count('All profiles')}</small></SidebarNavItem><SidebarNavItem active={section==='Favorites'} onClick={()=>navigate('Favorites')}><Icon name="heart"/>Favorites<small>{count('Favorites')}</small></SidebarNavItem></nav>
      ...
      <main className="ins-content">
        ...
        {section === 'Favorites' ? <>
          <div className="pl-section-intro"><h2>Your home lineup</h2><p>Five shortcuts, in your order. Reordering here updates the home cover flow.</p></div>
          <div className="pl-favorites">{favorites.map((id,index)=>{ const p=profiles.find(p=>p.id===id)!; return <article className={`pl-favorite ${id===loaded?'pl-favorite-loaded':''}`} key={id}>
            <button className="pl-favorite-preview" onClick={()=>setDetail(p)}><span className="pl-favorite-top"><span>0{index+1}</span>{loaded===id && <span className="pl-badge">Loaded</span>}</span><h2>{p.name}</h2><span className="pl-muted">{p.category}</span><Chart seed={id}/></button>
            <footer><div><button className="pl-round" disabled={index===0} aria-label={`Move ${p.name} earlier`} onClick={()=>move(index,-1)}><Icon name="up"/></button><button className="pl-round" disabled={index===favorites.length-1} aria-label={`Move ${p.name} later`} onClick={()=>move(index,1)}><Icon name="down"/></button></div><button className="pl-text" onClick={()=>setFavorites(favorites.filter(i=>i!==id))}>Remove</button></footer>
          ...
          <div className="pl-list-meta"><span>{visible.length} profiles</span><span>Open a profile to inspect it before loading</span></div>
          <section className="pl-list" aria-label="Profiles"><div className="pl-list-header"><span>Profile</span><span>Recipe</span><span>Temp.</span><span className="pl-desktop-source">Source</span><span/></div>
            {visible.map(p=><article className={`pl-row ${p.id===loaded?'pl-row-loaded':''}`} key={p.id}><button className="pl-row-open" onClick={()=>setDetail(p)} aria-label={`View ${p.name}`}><span className="pl-profile-cell"><span className="pl-thumbnail"><Chart seed={p.id}/></span><span><strong>{p.name}</strong><span className="pl-row-sub">{p.category}{p.id===loaded && <span className="pl-loaded-inline">● Loaded</span>}</span></span></span><span className="pl-recipe">{p.dose ? <>{p.dose}<small>g</small><span className="pl-recipe-arrow">→</span>{p.yield}<small>g</small></> : '—'}</span><span className="pl-temperature">{p.temp}°</span><span className="pl-desktop-source pl-muted">{p.source}</span></button><button className={`pl-fav-button ${favorites.includes(p.id)?'is-favorite':''}`} aria-label={`${favorites.includes(p.id)?'Unfavorite':'Favorite'} ${p.name}`} aria-pressed={favorites.includes(p.id)} onClick={()=>favorite(p)}><Icon name="heart"/></button></article>)}
      ...
      <header className="pl-detail-header"><button className="pl-round pl-close" onClick={()=>setDetail(null)} aria-label="Close profile detail">×</button><div><span className="pl-eyebrow">{detail.source.toUpperCase()} · {detail.category.toUpperCase()}</span><h1>{detail.name}</h1></div><div className="pl-actions"><button className="pl-button pl-primary" onClick={()=>{setLoaded(detail.id);setNotice('Profile loaded in this preview only')}}><Icon name="check"/>{loaded===detail.id?'Loaded':'Use profile'}</button><button className={`pl-button ${favorites.includes(detail.id)?'pl-selected':''}`} onClick={()=>favorite(detail)}><Icon name="heart"/>{favorites.includes(detail.id)?'Favorited':'Favorite'}</button><button className="pl-button" onClick={()=>setModal('edit')}><Icon name="edit"/>{detail.source==='Built-in'?'Edit a copy':'Edit'}</button>{detail.source!=='Built-in' && <button className="pl-button pl-danger" onClick={()=>setModal('delete')}>Delete</button>}</div></header>
      <div className="pl-detail-summary"><p>{detail.description}</p><span className="pl-muted">Recipe preview · illustrative targets</span></div>
      ...
      <section className="pl-target-chart"><header><h2>Extraction targets</h2><div className="pl-legend"><span><i/>Pressure</span><span><i/>Flow</span></div></header><div className="pl-chart-area"><div className="pl-axis"><span>9</span><span>6</span><span>3</span><span>0</span></div><Chart seed={detail.id} large/></div><div className="pl-time-axis"><span>0 s</span><span>10 s</span><span>20 s</span><span>30 s</span></div></section>
      <div className="pl-stage-heading"><h2>Stages</h2><span>Illustrative stage sequence</span></div><section className="pl-stages">{['Fill','Infuse','Extract'].map((name,i)=><article key={name}><header><span className="pl-stage-number">{i+1}</span><h3>{name}</h3><span>{[8,10,18][i]} s</span></header><div><Metric label={i===0?'Flow target':'Pressure target'} value={[4,2,6][i]} unit={i===0?'ml/s':'bar'}/><Metric label="Temperature" value={detail.temp} unit="°"/></div></article>)}</section>
    </main>}
